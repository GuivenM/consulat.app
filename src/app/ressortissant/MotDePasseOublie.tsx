...
export function MotDePasseOublieRessortissant() {
  const { motDePasseOublie } = useRessortissantAuth();
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      setMessage(await motDePasseOublie(email));
    } catch (err) {
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4">
      <div className="w-full max-w-md">
          ...
          {message ? (
            ...
            <form onSubmit={handleSubmit}>
              ...
              <p className="text-sm text-slate-500 mb-5">
                Indiquez l'email de votre espace consulaire : si un compte vérifié existe, vous recevrez un lien pour choisir un nouveau mot de passe.
              </p>
        ...
        <p className="text-center mt-6">
          <Link to="/espace-consulaire/login" className="text-slate-500 text-sm hover:text-slate-700 inline-flex items-center gap-1">
